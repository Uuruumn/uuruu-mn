...
export function ListingCard({ listing }: { listing: Listing }) {
  const imgSrc = listing.image_url || 'https://images.unsplash.com/photo-1494526585095-c41746248156?auto=format&fit=crop&w=1200&q=80';
  return (
    <article className="listing-card">
      ...
      <div className="card-body">
        ...
        <Link href={`/listings/${listing.id}`} className="card-title">{listing.title}</Link>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 16, flexWrap: 'wrap' }}>
          <Link href={`/listings/${listing.id}`} className="btn btn-ghost">Дэлгэрэнгүй</Link>
          <a href={`tel:${listing.phone}`} className="btn btn-primary">Утсаар холбогдох</a>
        </div>
      </div>
    </article>
  );
