...
async function uploadImage(supabase: any, userId: string, file: File, index: number): Promise<string | null> {
  try {
    ...
    if (error) { console.warn(`Image ${index} upload skipped:`, error.message); return null; }
    return supabase.storage.from('listing-images').getPublicUrl(fileName).data.publicUrl;
  } catch (e) { console.warn(`Image ${index} error:`, e); return null; }
}
...
export async function createListing(formData: FormData) {
  ...
  const landArea = landAreaRaw && String(landAreaRaw).trim() !== '' ? Number(landAreaRaw) : null;
  ...
  const imageFiles = (formData.getAll('images') as File[]).filter(f => f && f.size > 0).slice(0, 5);
  const imageUrls = (await Promise.all(
    imageFiles.map((file, i) => uploadImage(supabase, user.id, file, i + 1))
  ...
  const { error } = await supabase.from('listings').insert({
    user_id: user.id,
    title, listing_type: listingType, poster_type: posterType,
  ...
  try {
    resend.emails.send({
      ...
      html: `<h2>Шинэ зар бүртгэгдлээ</h2><p><strong>Гарчиг:</strong> ${title}</p><p><strong>Байршил:</strong> ${location}${district ? ` — ${district}` : ''}</p><p><strong>Үнэ:</strong> ${price.toLocaleString()}₮</p><p><strong>Утас:</strong> ${phone}</p>`,
    });
